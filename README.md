...
```sh
node fetch-venues.mjs                  # rebuilds public/venues.json from Playo (about a minute)
python3 -m http.server 8080 -d public  # any static file server works
```

Open http://localhost:8080. Opening `index.html` directly from disk does not work because browsers block `fetch` of `venues.json` over `file://`.
...
- `fetch-venues.mjs`: paginates Playo's venue list for football (`SP2`) around Challaghatta, fetches each venue's price chart, writes `public/venues.json`.
- `wrangler.jsonc`: Cloudflare deploy config, static assets only.
- `public/venues.json`: generated catalog. Sorted by distance from Challaghatta.
- `old/`: the previous Next.js/vinext attempt, kept only for reference. Delete when you no longer need it.
- `research/`: raw API samples and scripts from the original investigation.
...
The pricing endpoint intermittently returns 500 under concurrent load; the fetch script retries and falls back to the previous `venues.json` entry.
